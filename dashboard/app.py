...
def dashboard():
    try:
        ...
        status = r.json() if r.ok else {"error": f"Bot returned {r.status_code}"}
    except Exception as e:
        status = {"error": f"Couldn't reach bot: {e}"}
    return render_template("dashboard.html", status=status, bot_url=BOT_URL)
...
def editor_send():
    ...
    session["chat"] = history[-40:]  # cap session size
    return redirect(url_for("editor"))
